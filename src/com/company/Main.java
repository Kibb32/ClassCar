    ...
    public static void main(String[] args) throws Exception {
        ...
        while (isRun) {
            ...
            switch (action) {
                ...
                case 4:{
                    try {
                        int id = ConsoleHelper.InputInt("Введите ИД машины для изменения скорости: ");
                        int maxSpeed = ConsoleHelper.InputInt("Введите максимальную скорость (км/ч): ");
                        carsManager.getByIdCar(id).setMaxSpeed(maxSpeed);
                    } catch (Exception e) {
                        ConsoleHelper.PrintMessage(e.getMessage());
                    }
                        }
                ...
                case 5: {
                    try {
                        int id = ConsoleHelper.InputInt("Введите ИД машины для изменения цвета: ");
                        int colorIndex = ConsoleHelper.InputInt("Выберите новый цвет: ");
                        carsManager.getByIdCar(id).setColor(Car.Color.values()[colorIndex]);
                    } catch (Exception e) {
                        ConsoleHelper.PrintMessage(e.getMessage());
                    }
                }
